...
class PostController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $input=$request->all();
        // $this->postRepository->model()::where('id','>',0)->update(['publish_status'=>'已通过']);
    ...
    public function store(CreatePostRequest $request)
    {
        ...
        app('common')->postRepo()->adminPublishWithPost($post,auth('admin')->user());
    ...
    //快讯图片
    public function qkImgShow(Request $request,$post_id)
    {
        $post = $this->postRepository->findWithoutFail($post_id);
        ...
        $generate_img_url = app('common')->generateQuickNews($post,$request); 

        return app('common')->openImg($generate_img_url);
    }
    ...
    public function update($id, UpdatePostRequest $request)
    {
        ...
        if ( !array_key_exists('is_top', $input) ) {
            $input['is_top'] = 0;
        }
 ...
 */
                ...
                if (is_array($v) && $key != 'categories' ){
                    if(strpos($input['type'],",") !==false){
                        ...
                        if(in_array($customPostItemsSlug,$type)){
                            ...
                            if($this->postItemsRepository->findPostKey($post->id,$key)){
        ...
        if($post->author_type == 'user'){
             ...
             app('notice')->sendNoticeToUser(optional($post->AuthorWithType['author_obj'])->id,'您的文章'.a_link($post->name,'/post/'.$post->id).'审核状态已更新为'.$input['publish_status']);
